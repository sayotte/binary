...
func testRead(t *testing.T, order binary.ByteOrder, b []byte, s1 interface{}) {
	var s2 Struct
	bytesRead, err := Read(bytes.NewReader(b), order, &s2)
	if bytesRead != len(b) {
		t.Errorf("Read: expected to read %d bytes, instead %d", len(b), bytesRead)
	}
...
func TestReadSlice(t *testing.T) {
	slice := make([]int32, 2)
	_, err := Read(bytes.NewReader(src), binary.BigEndian, slice)
...
func TestSliceRoundTrip(t *testing.T) {
	...
	for _, array := range intArrays {
		...
		_, err = Read(buf, binary.BigEndian, dstSlice.Interface())
		if err != nil {
			t.Fatal(err)
		}
...
func TestBlankFields(t *testing.T) {
	...
	if err := binary.Write(buf, binary.LittleEndian, &b1); err != nil {
		t.Error(err)
	}

	// zero values must have been written for blank fields
	var p BlankFieldsProbe
	if _, err := Read(buf, binary.LittleEndian, &p); err != nil {
		t.Error(err)
	}
	...
	var b2 BlankFields
	if _, err := Read(buf, binary.LittleEndian, &b2); err != nil {
		t.Error(err)
	}
...
func TestUnexportedRead(t *testing.T) {
	...
	u1 := Unexported{a: 1}
	if err := binary.Write(&buf, binary.LittleEndian, &u1); err != nil {
		t.Fatal(err)
	}
	...
	var u2 Unexported
	Read(&buf, binary.LittleEndian, &u2)
}

func TestReadErrorMsg(t *testing.T) {
	var buf bytes.Buffer
	read := func(data interface{}) {
		_, err := Read(&buf, binary.LittleEndian, data)
...
func TestReadTruncated(t *testing.T) {
	...
	for i := 0; i <= len(data); i++ {
		...
		if _, err := Read(strings.NewReader(data[:i]), binary.LittleEndian, &b1); err != errWant {
			t.Errorf("Read(%d) with slice: got %v, want %v", i, err, errWant)
		}
		if _, err := Read(strings.NewReader(data[:i]), binary.LittleEndian, &b2); err != errWant {
